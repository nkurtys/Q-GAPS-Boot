...
def saveDatabase(table=None, query=None, M=False, L=False):
    #Access Database
    ...
    if (M == True):
        ...
        if query ==  None:
            ...
            #search user timeline and public hashtags for query
            results_query = App.search(query
                                    #, max_id=max_id
                                    )
            statuses = results_query["statuses"] + App.timeline_hashtag(query, limit=limit
                                                                        #, max_id=max_id
                                                                        )
    ...
    if (L == True):
        #create LIVIVO table
        r = requests.get('http://z3950.zbmed.de:6210/livivo?version=1.1&operation=searchRetrieve&query=' + table + '&maximumRecords=60&recordSchema=xml')
        data_dict = xmltodict.parse(r.text)
        records = data_dict['zs:searchRetrieveResponse']['zs:records']['zs:record']
        print(r.text)
        cursor.execute("CREATE TABLE IF NOT EXISTS " + table + "L" + " (TITLE, JOURNAL, sortyear, DBRECORDID)"
        )
        
        for record in records:
            try: 
                subtitle = " - " + record['zs:recordData']['doc']['UNTERTITEL']
            except:
                try: 
                    subtitle = " - " + record['zs:recordData']['doc']['SUBTITLE']
                except:
                    subtitle = ""
            title = record['zs:recordData']['doc']['TITLE'] + subtitle
            
            try:
                sortyear = record['zs:recordData']['doc']['sortyear']
            except:
                try:
                    sortyear = record['zs:recordData']['doc']['PUBLYEAR']
                except:
                        print(record)
                        sortyear = "NA"

            DBRECORDID = record['zs:recordData']['doc']['DBRECORDID']
            
            try:
                JOURNAL = record['zs:recordData']['doc']['JOURNAL']
            except:
                try:
                    JOURNAL = record['zs:recordData']['doc']['SOURCE']
                except:
                    try:
                        JOURNAL = record['zs:recordData']['doc']['D']
                    except:
            ...
            cursor.execute("INSERT INTO " + table + "L (TITLE, JOURNAL, sortyear, DBRECORDID) VALUES (?, ?, ?, ?)", (title, JOURNAL, sortyear, DBRECORDID))
            
                    #    + umfang + "," + "file.DATABASE.string" + "," 
                    #    + "file.ISBN.string" + "," + "file.IDENTIFIER.string" + "," + "file.DOCTYPE.string" + "," 
                    #    + "file.PUBLISHER.string" + "," + "file.REMARK.string" + "," 
                    #    + "file.VERFASSERANG.string" + "," + "file.RECORDDATE.string" + "," 
                    #    + "file.sortyear.string" + "," + "file.PUBLYEAR.string" + "," + "file.KEYWORDS.string" + "," 
                    #    + "file.bibkey.string" + "," + "file.UNTERTITEL.string" + "," 
                    #    + "file.LISTTAG.string" + "," + "file.SUBJECTCODE.string" + "," + "file.DOCUMENTURL.string" + "," 
                    #    + "file.DBRECORDID.string" + "," + "file.TITLE.string" + "," + "file.ABSTRACT.string" + "," 
                    #    + "file.classification.string" + "," + "file.MOREPERSONS.string" 
                    #    + ")")
        connection.commit()
        connection.close()
        print("Finished loading into the Database.") 
